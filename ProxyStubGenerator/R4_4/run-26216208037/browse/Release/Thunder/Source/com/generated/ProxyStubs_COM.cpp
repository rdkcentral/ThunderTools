...
namespace ProxyStubs {
    ...
    //
    // IIteratorTypeInstanceE6A923BB interface stub definitions
    //
    // Methods:
    ...
    ProxyStub::MethodHandler IteratorTypeInstanceE6A923BBStubMethods[] = {
        // (0) virtual bool Next(string&) = 0
        //
        ...
        , nullptr
    }; // IteratorTypeInstanceE6A923BBStubMethods

    //
    // IIteratorTypeInstance90D0727E interface stub definitions
    //
    // Methods:
    ...
    ProxyStub::MethodHandler IteratorTypeInstance90D0727EStubMethods[] = {
        // (0) virtual bool Next(uint32_t&) = 0
        //
        [](Core::ProxyType<Core::IPCChannel>& /* channel */, Core::ProxyType<RPC::InvokeMessage>& message) {
        ...
        , nullptr
    }; // IteratorTypeInstance90D0727EStubMethods
    ...
    ProxyStub::MethodHandler RemoteConnectionNotificationStubMethods[] = {
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            RPC::Data::Frame::Reader reader(message->Parameters().Reader());
            const Core::instance_id parameter_920468cfImplementation = reader.Number<Core::instance_id>();

            IRemoteConnection* _parameter_920468cf = nullptr;
            ProxyStub::UnknownProxy* parameter_920468cfProxy = nullptr;
            if (parameter_920468cfImplementation != 0) {
                parameter_920468cfProxy = RPC::Administrator::Instance().ProxyInstance(channel, parameter_920468cfImplementation, false, _parameter_920468cf);

                ASSERT((_parameter_920468cf != nullptr) && (parameter_920468cfProxy != nullptr));
            }

            implementation->Activated(_parameter_920468cf);

            if (parameter_920468cfProxy != nullptr) {
                RPC::Administrator::Instance().Release(parameter_920468cfProxy, message->Response());
            }
        },
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            RPC::Data::Frame::Reader reader(message->Parameters().Reader());
            const Core::instance_id parameter_f3dcf4e8Implementation = reader.Number<Core::instance_id>();

            IRemoteConnection* _parameter_f3dcf4e8 = nullptr;
            ProxyStub::UnknownProxy* parameter_f3dcf4e8Proxy = nullptr;
            if (parameter_f3dcf4e8Implementation != 0) {
                parameter_f3dcf4e8Proxy = RPC::Administrator::Instance().ProxyInstance(channel, parameter_f3dcf4e8Implementation, false, _parameter_f3dcf4e8);

                ASSERT((_parameter_f3dcf4e8 != nullptr) && (parameter_f3dcf4e8Proxy != nullptr));
            }

            implementation->Deactivated(_parameter_f3dcf4e8);

            if (parameter_f3dcf4e8Proxy != nullptr) {
                RPC::Administrator::Instance().Release(parameter_f3dcf4e8Proxy, message->Response());
            }
        },
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            RPC::Data::Frame::Reader reader(message->Parameters().Reader());
            const Core::instance_id parameter_c914d66fImplementation = reader.Number<Core::instance_id>();

            IRemoteConnection* _parameter_c914d66f = nullptr;
            ProxyStub::UnknownProxy* parameter_c914d66fProxy = nullptr;
            if (parameter_c914d66fImplementation != 0) {
                parameter_c914d66fProxy = RPC::Administrator::Instance().ProxyInstance(channel, parameter_c914d66fImplementation, false, _parameter_c914d66f);

                ASSERT((_parameter_c914d66f != nullptr) && (parameter_c914d66fProxy != nullptr));
            }

            implementation->Terminated(_parameter_c914d66f);

            if (parameter_c914d66fProxy != nullptr) {
                RPC::Administrator::Instance().Release(parameter_c914d66fProxy, message->Response());
            }
        }
        , nullptr
    ...
    //
    // IIteratorTypeInstanceE6A923BB interface proxy definitions
    //
    // Methods:
    //  (0) virtual bool Next(string&) = 0
    //  (1) virtual bool Previous(string&) = 0
    ...
    class IteratorTypeInstanceE6A923BBProxy final : public ProxyStub::UnknownProxyType<IIteratorType<string, ID_STRINGITERATOR>> {
    public:
        IteratorTypeInstanceE6A923BBProxy(const Core::ProxyType<Core::IPCChannel>& channel, const Core::instance_id implementation, const bool otherSideInformed)
    ...
    }; // class IteratorTypeInstanceE6A923BBProxy

    //
    // IIteratorTypeInstance90D0727E interface proxy definitions
    //
    // Methods:
    //  (0) virtual bool Next(uint32_t&) = 0
    //  (1) virtual bool Previous(uint32_t&) = 0
    ...
    class IteratorTypeInstance90D0727EProxy final : public ProxyStub::UnknownProxyType<IIteratorType<uint32_t, ID_VALUEITERATOR>> {
    public:
        IteratorTypeInstance90D0727EProxy(const Core::ProxyType<Core::IPCChannel>& channel, const Core::instance_id implementation, const bool otherSideInformed)
            : BaseClass(channel, implementation, otherSideInformed)
        {
        }
    ...
    }; // class IteratorTypeInstance90D0727EProxy
    ...
    public:
        ...
        void Activated(IRemoteConnection* _parameter_920468cf) override
        {
            IPCMessage message(BaseClass::Message(0));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_920468cf));
        ...
        void Deactivated(IRemoteConnection* _parameter_f3dcf4e8) override
        {
            IPCMessage message(BaseClass::Message(1));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_f3dcf4e8));
        ...
        void Terminated(IRemoteConnection* _parameter_c914d66f) override
        {
            IPCMessage message(BaseClass::Message(2));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_c914d66f));
    ...
    namespace {
        ...
        typedef ProxyStub::UnknownStubType<IIteratorType<string, ID_STRINGITERATOR>, IteratorTypeInstanceE6A923BBStubMethods> IteratorTypeInstanceE6A923BBStub;
        typedef ProxyStub::UnknownStubType<IIteratorType<uint32_t, ID_VALUEITERATOR>, IteratorTypeInstance90D0727EStubMethods> IteratorTypeInstance90D0727EStub;
        ...
        public:
            Instantiation()
            {
                RPC::Administrator::Instance().Announce<IIteratorType<string, ID_STRINGITERATOR>, IteratorTypeInstanceE6A923BBProxy, IteratorTypeInstanceE6A923BBStub>();
                RPC::Administrator::Instance().Announce<IIteratorType<uint32_t, ID_VALUEITERATOR>, IteratorTypeInstance90D0727EProxy, IteratorTypeInstance90D0727EStub>();

...
namespace ProxyStubs {
    ...
    ProxyStub::MethodHandler CompositPluginStubMethods[] = {
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            RPC::Data::Frame::Reader reader(message->Parameters().Reader());
            const Core::instance_id parameter_9f3990d7Implementation = reader.Number<Core::instance_id>();

            ICompositPlugin::INotification* _parameter_9f3990d7 = nullptr;
            ProxyStub::UnknownProxy* parameter_9f3990d7Proxy = nullptr;
            if (parameter_9f3990d7Implementation != 0) {
                parameter_9f3990d7Proxy = RPC::Administrator::Instance().ProxyInstance(channel, parameter_9f3990d7Implementation, false, _parameter_9f3990d7);

                ASSERT((_parameter_9f3990d7 != nullptr) && (parameter_9f3990d7Proxy != nullptr));
            }

            uint32_t result = implementation->Register(_parameter_9f3990d7);

            RPC::Data::Frame::Writer writer(message->Response().Writer());
            writer.Number<uint32_t>(result);

            if (parameter_9f3990d7Proxy != nullptr) {
                RPC::Administrator::Instance().Release(parameter_9f3990d7Proxy, message->Response());
            }
        },
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            RPC::Data::Frame::Reader reader(message->Parameters().Reader());
            const Core::instance_id parameter_55b38e6dImplementation = reader.Number<Core::instance_id>();

            ICompositPlugin::INotification* _parameter_55b38e6d = nullptr;
            ProxyStub::UnknownProxy* parameter_55b38e6dProxy = nullptr;
            if (parameter_55b38e6dImplementation != 0) {
                parameter_55b38e6dProxy = RPC::Administrator::Instance().ProxyInstance(channel, parameter_55b38e6dImplementation, false, _parameter_55b38e6d);

                ASSERT((_parameter_55b38e6d != nullptr) && (parameter_55b38e6dProxy != nullptr));
            }

            uint32_t result = implementation->Unregister(_parameter_55b38e6d);

            RPC::Data::Frame::Writer writer(message->Response().Writer());
            writer.Number<uint32_t>(result);

            if (parameter_55b38e6dProxy != nullptr) {
                RPC::Administrator::Instance().Release(parameter_55b38e6dProxy, message->Response());
            }
        }
        , nullptr
    ...
    public:
        ...
        uint32_t Register(ICompositPlugin::INotification* _parameter_9f3990d7) override
        {
            IPCMessage message(BaseClass::Message(0));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_9f3990d7));
        ...
        uint32_t Unregister(ICompositPlugin::INotification* _parameter_55b38e6d) override
        {
            IPCMessage message(BaseClass::Message(1));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_55b38e6d));

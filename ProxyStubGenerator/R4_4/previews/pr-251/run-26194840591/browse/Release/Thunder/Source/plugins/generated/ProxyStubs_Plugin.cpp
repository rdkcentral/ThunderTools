...
namespace ProxyStubs {
    ...
    ProxyStub::MethodHandler CompositPluginStubMethods[] = {
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            RPC::Data::Frame::Reader reader(message->Parameters().Reader());
            const Core::instance_id parameter_9eedb446Implementation = reader.Number<Core::instance_id>();

            ICompositPlugin::INotification* _parameter_9eedb446 = nullptr;
            ProxyStub::UnknownProxy* parameter_9eedb446Proxy = nullptr;
            if (parameter_9eedb446Implementation != 0) {
                parameter_9eedb446Proxy = RPC::Administrator::Instance().ProxyInstance(channel, parameter_9eedb446Implementation, false, _parameter_9eedb446);

                ASSERT((_parameter_9eedb446 != nullptr) && (parameter_9eedb446Proxy != nullptr));
            }

            uint32_t result = implementation->Register(_parameter_9eedb446);

            RPC::Data::Frame::Writer writer(message->Response().Writer());
            writer.Number<uint32_t>(result);

            if (parameter_9eedb446Proxy != nullptr) {
                RPC::Administrator::Instance().Release(parameter_9eedb446Proxy, message->Response());
            }
        },
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            RPC::Data::Frame::Reader reader(message->Parameters().Reader());
            const Core::instance_id parameter_72d0d3a0Implementation = reader.Number<Core::instance_id>();

            ICompositPlugin::INotification* _parameter_72d0d3a0 = nullptr;
            ProxyStub::UnknownProxy* parameter_72d0d3a0Proxy = nullptr;
            if (parameter_72d0d3a0Implementation != 0) {
                parameter_72d0d3a0Proxy = RPC::Administrator::Instance().ProxyInstance(channel, parameter_72d0d3a0Implementation, false, _parameter_72d0d3a0);

                ASSERT((_parameter_72d0d3a0 != nullptr) && (parameter_72d0d3a0Proxy != nullptr));
            }

            uint32_t result = implementation->Unregister(_parameter_72d0d3a0);

            RPC::Data::Frame::Writer writer(message->Response().Writer());
            writer.Number<uint32_t>(result);

            if (parameter_72d0d3a0Proxy != nullptr) {
                RPC::Administrator::Instance().Release(parameter_72d0d3a0Proxy, message->Response());
            }
        }
        , nullptr
    ...
    public:
        ...
        uint32_t Register(ICompositPlugin::INotification* _parameter_9eedb446) override
        {
            IPCMessage message(BaseClass::Message(0));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_9eedb446));
        ...
        uint32_t Unregister(ICompositPlugin::INotification* _parameter_72d0d3a0) override
        {
            IPCMessage message(BaseClass::Message(1));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_72d0d3a0));

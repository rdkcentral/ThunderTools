...
namespace ProxyStubs {
    ...
    //
    // IIteratorTypeInstance04A72B79 interface stub definitions
    //
    // Methods:
    ...
    ProxyStub::MethodHandler IteratorTypeInstance04A72B79StubMethods[] = {
        // (0) virtual bool Next(string&) = 0
        //
        ...
        , nullptr
    }; // IteratorTypeInstance04A72B79StubMethods

    //
    // IIteratorTypeInstance081AC06A interface stub definitions
    //
    // Methods:
    ...
    ProxyStub::MethodHandler IteratorTypeInstance081AC06AStubMethods[] = {
        // (0) virtual bool Next(uint32_t&) = 0
        //
        [](Core::ProxyType<Core::IPCChannel>& /* channel */, Core::ProxyType<RPC::InvokeMessage>& message) {
        ...
        , nullptr
    }; // IteratorTypeInstance081AC06AStubMethods
    ...
    ProxyStub::MethodHandler RemoteConnectionNotificationStubMethods[] = {
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            RPC::Data::Frame::Reader reader(message->Parameters().Reader());
            const Core::instance_id parameter_3bddbeb9Implementation = reader.Number<Core::instance_id>();

            IRemoteConnection* _parameter_3bddbeb9 = nullptr;
            ProxyStub::UnknownProxy* parameter_3bddbeb9Proxy = nullptr;
            if (parameter_3bddbeb9Implementation != 0) {
                parameter_3bddbeb9Proxy = RPC::Administrator::Instance().ProxyInstance(channel, parameter_3bddbeb9Implementation, false, _parameter_3bddbeb9);

                ASSERT((_parameter_3bddbeb9 != nullptr) && (parameter_3bddbeb9Proxy != nullptr));
            }

            implementation->Activated(_parameter_3bddbeb9);

            if (parameter_3bddbeb9Proxy != nullptr) {
                RPC::Administrator::Instance().Release(parameter_3bddbeb9Proxy, message->Response());
            }
        },
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            RPC::Data::Frame::Reader reader(message->Parameters().Reader());
            const Core::instance_id parameter_f9bed6e8Implementation = reader.Number<Core::instance_id>();

            IRemoteConnection* _parameter_f9bed6e8 = nullptr;
            ProxyStub::UnknownProxy* parameter_f9bed6e8Proxy = nullptr;
            if (parameter_f9bed6e8Implementation != 0) {
                parameter_f9bed6e8Proxy = RPC::Administrator::Instance().ProxyInstance(channel, parameter_f9bed6e8Implementation, false, _parameter_f9bed6e8);

                ASSERT((_parameter_f9bed6e8 != nullptr) && (parameter_f9bed6e8Proxy != nullptr));
            }

            implementation->Deactivated(_parameter_f9bed6e8);

            if (parameter_f9bed6e8Proxy != nullptr) {
                RPC::Administrator::Instance().Release(parameter_f9bed6e8Proxy, message->Response());
            }
        },
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            RPC::Data::Frame::Reader reader(message->Parameters().Reader());
            const Core::instance_id parameter_e6ab710aImplementation = reader.Number<Core::instance_id>();

            IRemoteConnection* _parameter_e6ab710a = nullptr;
            ProxyStub::UnknownProxy* parameter_e6ab710aProxy = nullptr;
            if (parameter_e6ab710aImplementation != 0) {
                parameter_e6ab710aProxy = RPC::Administrator::Instance().ProxyInstance(channel, parameter_e6ab710aImplementation, false, _parameter_e6ab710a);

                ASSERT((_parameter_e6ab710a != nullptr) && (parameter_e6ab710aProxy != nullptr));
            }

            implementation->Terminated(_parameter_e6ab710a);

            if (parameter_e6ab710aProxy != nullptr) {
                RPC::Administrator::Instance().Release(parameter_e6ab710aProxy, message->Response());
            }
        }
        , nullptr
    ...
    //
    // IIteratorTypeInstance04A72B79 interface proxy definitions
    //
    // Methods:
    //  (0) virtual bool Next(string&) = 0
    //  (1) virtual bool Previous(string&) = 0
    ...
    class IteratorTypeInstance04A72B79Proxy final : public ProxyStub::UnknownProxyType<IIteratorType<string, ID_STRINGITERATOR>> {
    public:
        IteratorTypeInstance04A72B79Proxy(const Core::ProxyType<Core::IPCChannel>& channel, const Core::instance_id implementation, const bool otherSideInformed)
    ...
    }; // class IteratorTypeInstance04A72B79Proxy

    //
    // IIteratorTypeInstance081AC06A interface proxy definitions
    //
    // Methods:
    //  (0) virtual bool Next(uint32_t&) = 0
    //  (1) virtual bool Previous(uint32_t&) = 0
    ...
    class IteratorTypeInstance081AC06AProxy final : public ProxyStub::UnknownProxyType<IIteratorType<uint32_t, ID_VALUEITERATOR>> {
    public:
        IteratorTypeInstance081AC06AProxy(const Core::ProxyType<Core::IPCChannel>& channel, const Core::instance_id implementation, const bool otherSideInformed)
            : BaseClass(channel, implementation, otherSideInformed)
        {
        }
    ...
    }; // class IteratorTypeInstance081AC06AProxy
    ...
    public:
        ...
        void Activated(IRemoteConnection* _parameter_3bddbeb9) override
        {
            IPCMessage message(BaseClass::Message(0));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_3bddbeb9));
        ...
        void Deactivated(IRemoteConnection* _parameter_f9bed6e8) override
        {
            IPCMessage message(BaseClass::Message(1));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_f9bed6e8));
        ...
        void Terminated(IRemoteConnection* _parameter_e6ab710a) override
        {
            IPCMessage message(BaseClass::Message(2));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_e6ab710a));
    ...
    namespace {
        ...
        typedef ProxyStub::UnknownStubType<IIteratorType<string, ID_STRINGITERATOR>, IteratorTypeInstance04A72B79StubMethods> IteratorTypeInstance04A72B79Stub;
        typedef ProxyStub::UnknownStubType<IIteratorType<uint32_t, ID_VALUEITERATOR>, IteratorTypeInstance081AC06AStubMethods> IteratorTypeInstance081AC06AStub;
        ...
        public:
            Instantiation()
            {
                RPC::Administrator::Instance().Announce<IIteratorType<string, ID_STRINGITERATOR>, IteratorTypeInstance04A72B79Proxy, IteratorTypeInstance04A72B79Stub>();
                RPC::Administrator::Instance().Announce<IIteratorType<uint32_t, ID_VALUEITERATOR>, IteratorTypeInstance081AC06AProxy, IteratorTypeInstance081AC06AStub>();

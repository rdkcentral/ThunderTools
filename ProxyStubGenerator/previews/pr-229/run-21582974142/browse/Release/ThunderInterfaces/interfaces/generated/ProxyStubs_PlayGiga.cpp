...
namespace ProxyStubs {
    ...
    static ProxyStub::MethodHandler ExchangePlayGigaStubMethods[] = {
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            hresult = [&]() -> Core::hresult {
                ...
                const uint16_t _parameter_14dcb5e6PeekedLen__ = reader.PeekNumber<uint16_t>();
                if (reader.Length() < (static_cast<uint32_t>(Core::RealSize<uint16_t>()) + _parameter_14dcb5e6PeekedLen__)) { return (COM_ERROR | Core::ERROR_READ_ERROR); }
                const string _parameter_14dcb5e6 = reader.Text();
                if (reader.Length() < (Core::RealSize<uint16_t>())) { return (COM_ERROR | Core::ERROR_READ_ERROR); }
                const uint16_t _parameter_198bb5c2PeekedLen__ = reader.PeekNumber<uint16_t>();
                if (reader.Length() < (static_cast<uint32_t>(Core::RealSize<uint16_t>()) + _parameter_198bb5c2PeekedLen__)) { return (COM_ERROR | Core::ERROR_READ_ERROR); }
                const string _parameter_198bb5c2 = reader.Text();

                implementation->Launch(static_cast<const string&>(_parameter_14dcb5e6), static_cast<const string&>(_parameter_198bb5c2));
    ...
    public:
        ...
        void Launch(const string& _parameter_14dcb5e6, const string& _parameter_198bb5c2) override
        {
            IPCMessage message(static_cast<const ProxyStub::UnknownProxy&>(*this).Message(0));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Text(_parameter_14dcb5e6);
            writer.Text(_parameter_198bb5c2);

...
namespace ProxyStubs {
    ...
    static ProxyStub::MethodHandler ExchangeSessionStubMethods[] = {
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            hresult = [&]() -> Core::hresult {
                ...
                RPC::Data::Frame::Reader reader(message->Parameters().Reader());
                if (reader.Length() < (Core::RealSize<uint16_t>())) { return (COM_ERROR | Core::ERROR_READ_ERROR); }
                const uint16_t _parameter_62c6fcdbPeekedLen__ = reader.PeekNumber<uint16_t>();
                if (reader.Length() < (static_cast<uint32_t>(Core::RealSize<uint16_t>()) + _parameter_62c6fcdbPeekedLen__)) { return (COM_ERROR | Core::ERROR_READ_ERROR); }
                const string _parameter_62c6fcdb = reader.Text();
                if (reader.Length() < (Core::RealSize<uint16_t>())) { return (COM_ERROR | Core::ERROR_READ_ERROR); }
                const uint16_t _parameter_574f16b3PeekedLen__ = reader.PeekNumber<uint16_t>();
                if (reader.Length() < (static_cast<uint32_t>(Core::RealSize<uint16_t>()) + _parameter_574f16b3PeekedLen__)) { return (COM_ERROR | Core::ERROR_READ_ERROR); }
                const string _parameter_574f16b3 = reader.Text();

                Exchange::OCDM_RESULT result = implementation->SetParameter(static_cast<const string&>(_parameter_62c6fcdb), static_cast<const string&>(_parameter_574f16b3));
    ...
    public:
        ...
        Exchange::OCDM_RESULT SetParameter(const string& _parameter_62c6fcdb, const string& _parameter_574f16b3) override
        {
            IPCMessage message(static_cast<const ProxyStub::UnknownProxy&>(*this).Message(13));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Text(_parameter_62c6fcdb);
            writer.Text(_parameter_574f16b3);

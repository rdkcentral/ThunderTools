...
namespace ProxyStubs {
    ...
    static ProxyStub::MethodHandler CompositPluginStubMethods[] = {
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            hresult = [&]() -> Core::hresult {
                ...
                RPC::Data::Frame::Reader reader(message->Parameters().Reader());
                if (reader.Length() < (sizeof(Core::instance_id))) { return (COM_ERROR | Core::ERROR_READ_ERROR); }
                Core::instance_id _parameter_966ae9cbInstanceId__ = reader.Number<Core::instance_id>();

                ICompositPlugin::ICallback* _parameter_966ae9cb{};
                ProxyStub::UnknownProxy* _parameter_966ae9cbProxy__ = nullptr;
                if (_parameter_966ae9cbInstanceId__ != 0) {
                    _parameter_966ae9cbProxy__ = RPC::Administrator::Instance().ProxyInstance(channel, _parameter_966ae9cbInstanceId__, false, _parameter_966ae9cb);
                    ASSERT((_parameter_966ae9cb != nullptr) && (_parameter_966ae9cbProxy__ != nullptr));
                    if ((_parameter_966ae9cb == nullptr) || (_parameter_966ae9cbProxy__ == nullptr)) { return (COM_ERROR | Core::ERROR_NOT_EXIST); }
                }

                uint32_t result = implementation->Callback(_parameter_966ae9cb);

                RPC::Data::Frame::Writer writer(message->Response().Writer());
                writer.Number<uint32_t>(result);

                if (_parameter_966ae9cbProxy__ != nullptr) {
                    RPC::Administrator::Instance().Release(_parameter_966ae9cbProxy__, message->Response());
                }
    ...
    public:
        ...
        uint32_t Callback(ICompositPlugin::ICallback* _parameter_966ae9cb) override
        {
            IPCMessage message(static_cast<const ProxyStub::UnknownProxy&>(*this).Message(0));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_966ae9cb));

            const RPC::InstanceRecord passedInstances[] = { { RPC::instance_cast(_parameter_966ae9cb), ICompositPlugin::ICallback::ID }, { 0, 0 } };
            static_cast<const ProxyStub::UnknownProxy&>(*this).Channel()->CustomData(passedInstances);

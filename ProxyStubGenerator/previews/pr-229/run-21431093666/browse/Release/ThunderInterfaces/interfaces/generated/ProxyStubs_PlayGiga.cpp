...
namespace ProxyStubs {
    ...
    static ProxyStub::MethodHandler ExchangePlayGigaStubMethods[] = {
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            hresult = [&]() -> Core::hresult {
                ...
                const uint16_t _parameter_d5d91eb8PeekedLen__ = reader.PeekNumber<uint16_t>();
                if (reader.Length() < (static_cast<uint32_t>(Core::RealSize<uint16_t>()) + _parameter_d5d91eb8PeekedLen__)) { return (COM_ERROR | Core::ERROR_READ_ERROR); }
                const string _parameter_d5d91eb8 = reader.Text();
                if (reader.Length() < (Core::RealSize<uint16_t>())) { return (COM_ERROR | Core::ERROR_READ_ERROR); }
                const uint16_t _parameter_0cd38ceePeekedLen__ = reader.PeekNumber<uint16_t>();
                if (reader.Length() < (static_cast<uint32_t>(Core::RealSize<uint16_t>()) + _parameter_0cd38ceePeekedLen__)) { return (COM_ERROR | Core::ERROR_READ_ERROR); }
                const string _parameter_0cd38cee = reader.Text();

                implementation->Launch(static_cast<const string&>(_parameter_d5d91eb8), static_cast<const string&>(_parameter_0cd38cee));
    ...
    public:
        ...
        void Launch(const string& _parameter_d5d91eb8, const string& _parameter_0cd38cee) override
        {
            IPCMessage message(static_cast<const ProxyStub::UnknownProxy&>(*this).Message(0));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Text(_parameter_d5d91eb8);
            writer.Text(_parameter_0cd38cee);

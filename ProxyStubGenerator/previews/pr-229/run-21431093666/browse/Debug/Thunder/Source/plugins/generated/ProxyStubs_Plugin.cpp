...
namespace ProxyStubs {
    ...
    static ProxyStub::MethodHandler CompositPluginStubMethods[] = {
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            RPC::Data::Frame::Reader reader(message->Parameters().Reader());
            Core::instance_id _parameter_46308d8aInstanceId__ = reader.Number<Core::instance_id>();

            ICompositPlugin::ICallback* _parameter_46308d8a{};
            ProxyStub::UnknownProxy* _parameter_46308d8aProxy__ = nullptr;
            if (_parameter_46308d8aInstanceId__ != 0) {
                _parameter_46308d8aProxy__ = RPC::Administrator::Instance().ProxyInstance(channel, _parameter_46308d8aInstanceId__, false, _parameter_46308d8a);
                ASSERT((_parameter_46308d8a != nullptr) && (_parameter_46308d8aProxy__ != nullptr));
            }

            uint32_t result = implementation->Callback(_parameter_46308d8a);

            RPC::Data::Frame::Writer writer(message->Response().Writer());
            writer.Number<uint32_t>(result);

            if (_parameter_46308d8aProxy__ != nullptr) {
                RPC::Administrator::Instance().Release(_parameter_46308d8aProxy__, message->Response());
            }
        }
        , nullptr
    ...
    public:
        ...
        uint32_t Callback(ICompositPlugin::ICallback* _parameter_46308d8a) override
        {
            IPCMessage message(static_cast<const ProxyStub::UnknownProxy&>(*this).Message(0));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_46308d8a));

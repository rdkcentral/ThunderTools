...
namespace ProxyStubs {
    ...
    static ProxyStub::MethodHandler CompositPluginStubMethods[] = {
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            RPC::Data::Frame::Reader reader(message->Parameters().Reader());
            Core::instance_id _parameter_ea0afefbInstanceId__ = reader.Number<Core::instance_id>();

            ICompositPlugin::ICallback* _parameter_ea0afefb{};
            ProxyStub::UnknownProxy* _parameter_ea0afefbProxy__ = nullptr;
            if (_parameter_ea0afefbInstanceId__ != 0) {
                _parameter_ea0afefbProxy__ = RPC::Administrator::Instance().ProxyInstance(channel, _parameter_ea0afefbInstanceId__, false, _parameter_ea0afefb);
                ASSERT((_parameter_ea0afefb != nullptr) && (_parameter_ea0afefbProxy__ != nullptr));
            }

            uint32_t result = implementation->Callback(_parameter_ea0afefb);

            RPC::Data::Frame::Writer writer(message->Response().Writer());
            writer.Number<uint32_t>(result);

            if (_parameter_ea0afefbProxy__ != nullptr) {
                RPC::Administrator::Instance().Release(_parameter_ea0afefbProxy__, message->Response());
            }
        }
        , nullptr
    ...
    public:
        ...
        uint32_t Callback(ICompositPlugin::ICallback* _parameter_ea0afefb) override
        {
            IPCMessage message(static_cast<const ProxyStub::UnknownProxy&>(*this).Message(0));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_ea0afefb));

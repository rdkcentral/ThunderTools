...
namespace ProxyStubs {
    ...
    static ProxyStub::MethodHandler ExchangePlayGigaStubMethods[] = {
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            hresult = [&]() -> Core::hresult {
                ...
                const uint16_t _parameter_8aa914bcPeekedLen__ = reader.PeekNumber<uint16_t>();
                if (reader.Length() < (static_cast<uint32_t>(Core::RealSize<uint16_t>()) + _parameter_8aa914bcPeekedLen__)) { return (COM_ERROR | Core::ERROR_READ_ERROR); }
                const string _parameter_8aa914bc = reader.Text();
                if (reader.Length() < (Core::RealSize<uint16_t>())) { return (COM_ERROR | Core::ERROR_READ_ERROR); }
                const uint16_t _parameter_6da6e014PeekedLen__ = reader.PeekNumber<uint16_t>();
                if (reader.Length() < (static_cast<uint32_t>(Core::RealSize<uint16_t>()) + _parameter_6da6e014PeekedLen__)) { return (COM_ERROR | Core::ERROR_READ_ERROR); }
                const string _parameter_6da6e014 = reader.Text();

                implementation->Launch(static_cast<const string&>(_parameter_8aa914bc), static_cast<const string&>(_parameter_6da6e014));
    ...
    public:
        ...
        void Launch(const string& _parameter_8aa914bc, const string& _parameter_6da6e014) override
        {
            IPCMessage message(static_cast<const ProxyStub::UnknownProxy&>(*this).Message(0));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Text(_parameter_8aa914bc);
            writer.Text(_parameter_6da6e014);

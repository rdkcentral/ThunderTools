...
namespace ProxyStubs {
    ...
    static ProxyStub::MethodHandler CompositPluginStubMethods[] = {
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            hresult = [&]() -> Core::hresult {
                ...
                RPC::Data::Frame::Reader reader(message->Parameters().Reader());
                if (reader.Length() < (sizeof(Core::instance_id))) { return (COM_ERROR | Core::ERROR_READ_ERROR); }
                Core::instance_id _parameter_f93b4e6dInstanceId__ = reader.Number<Core::instance_id>();

                ICompositPlugin::ICallback* _parameter_f93b4e6d{};
                ProxyStub::UnknownProxy* _parameter_f93b4e6dProxy__ = nullptr;
                if (_parameter_f93b4e6dInstanceId__ != 0) {
                    _parameter_f93b4e6dProxy__ = RPC::Administrator::Instance().ProxyInstance(channel, _parameter_f93b4e6dInstanceId__, false, _parameter_f93b4e6d);
                    ASSERT((_parameter_f93b4e6d != nullptr) && (_parameter_f93b4e6dProxy__ != nullptr));
                    if ((_parameter_f93b4e6d == nullptr) || (_parameter_f93b4e6dProxy__ == nullptr)) { return (COM_ERROR | Core::ERROR_NOT_EXIST); }
                }

                uint32_t result = implementation->Callback(_parameter_f93b4e6d);

                RPC::Data::Frame::Writer writer(message->Response().Writer());
                writer.Number<uint32_t>(result);

                if (_parameter_f93b4e6dProxy__ != nullptr) {
                    RPC::Administrator::Instance().Release(_parameter_f93b4e6dProxy__, message->Response());
                }
    ...
    public:
        ...
        uint32_t Callback(ICompositPlugin::ICallback* _parameter_f93b4e6d) override
        {
            IPCMessage message(static_cast<const ProxyStub::UnknownProxy&>(*this).Message(0));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_f93b4e6d));

            const RPC::InstanceRecord passedInstances[] = { { RPC::instance_cast(_parameter_f93b4e6d), ICompositPlugin::ICallback::ID }, { 0, 0 } };
            static_cast<const ProxyStub::UnknownProxy&>(*this).Channel()->CustomData(passedInstances);

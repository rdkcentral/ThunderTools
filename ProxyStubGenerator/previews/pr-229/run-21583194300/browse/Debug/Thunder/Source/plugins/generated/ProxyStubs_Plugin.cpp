...
namespace ProxyStubs {
    ...
    static ProxyStub::MethodHandler CompositPluginStubMethods[] = {
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            RPC::Data::Frame::Reader reader(message->Parameters().Reader());
            Core::instance_id _parameter_6838e8d1InstanceId__ = reader.Number<Core::instance_id>();

            ICompositPlugin::ICallback* _parameter_6838e8d1{};
            ProxyStub::UnknownProxy* _parameter_6838e8d1Proxy__ = nullptr;
            if (_parameter_6838e8d1InstanceId__ != 0) {
                _parameter_6838e8d1Proxy__ = RPC::Administrator::Instance().ProxyInstance(channel, _parameter_6838e8d1InstanceId__, false, _parameter_6838e8d1);
                ASSERT((_parameter_6838e8d1 != nullptr) && (_parameter_6838e8d1Proxy__ != nullptr));
            }

            uint32_t result = implementation->Callback(_parameter_6838e8d1);

            RPC::Data::Frame::Writer writer(message->Response().Writer());
            writer.Number<uint32_t>(result);

            if (_parameter_6838e8d1Proxy__ != nullptr) {
                RPC::Administrator::Instance().Release(_parameter_6838e8d1Proxy__, message->Response());
            }
        }
        , nullptr
    ...
    public:
        ...
        uint32_t Callback(ICompositPlugin::ICallback* _parameter_6838e8d1) override
        {
            IPCMessage message(static_cast<const ProxyStub::UnknownProxy&>(*this).Message(0));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_6838e8d1));

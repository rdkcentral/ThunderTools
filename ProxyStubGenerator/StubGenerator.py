...
def GenerateStubs2(output_file, source_file, includePaths = [], defaults = "", extra_includes = [], scan_only=False):
    ...
    with open(output_file, "w") as file:
        ...
        class EmitIdentifier():
            def __init__(self, index, interface, identifier, override_name=None, suppress_type=False):
                ...
                if self.max_length_of and (self.max_length != self.length) and not no_length_warnings:
                    ...
                    if self.kind.size == "long long":
                        ...
                        log.WarnLine(self.identifier, "'%s': long int buffer max-length is supported, but buffers up to 64 KB are recommended" % self.trace_proto)
                    if not self.kind.fixed:
                ...
                if ENABLE_SECURE:
                    ...
                    if self.restrict_range:
                        ...
                        elif (self.is_buffer or self.is_string) and self.restrict_range[0] < 0:
                    ...
                    if self.is_string:
                        aux_size = ("uint32_t" if self.is_ccstring else "uint16_t")
            ...
            def read_rpc_type(self):
                ...
                # Strings
                    ...
                    return "Text<uint32_t>()" if self.is_ccstring else "Text()"
        ...
        def CheckFrame(p, by_parameter=False):
            if ENABLE_SECURE:
                emit.Line()
                emit.Line("if (%s.Length() < (%s)) { return (0x80000000 | Core::ERROR_INVALID_INPUT_LENGTH); }" % \
                            (vars["reader"], (p.as_rvalue if by_parameter else p.storage_size)))

        def CheckRange(p, q = p):
            if ENABLE_SECURE and p.restrict_range:
                emit.Line("if (!((%s >= %s) && (%s <= %s))) { return (0x80000000 | Core::ERROR_INVALID_RANGE) }" % \
                            (q.as_rvalue, p.restrict_range[0], q.as_rvalue, p.restrict_range[1]))

        def CheckSize(p):
            if ENABLE_SECURE:
                emit.Line("%s = %s.PeekNumber<%s>();" % (p.peek_length.as_temporary, vars["reader"], p.peek_length.type_name))
                CheckRange(p, p.peek_length)
                emit.Line("if (%s.Length() < (%s + %s)) { return (0x80000000 | Core::ERROR_INVALID_INPUT_LENGTH); }" % \
        ...
        def EmitStubMethodImplementation(index, method, interface_name, interface, retval, params, \
            ...
            def AcquireInterface(p):
                ...
                if ENABLE_SECURE:
                    emit.Line("if ((%s == nullptr) || (%s == nullptr)) { return (0x80000000 | Core::ERROR_NOT_EXIST); }" % (p.proxy_instance, p.proxy))
                else:
            ...
            if ENABLE_SECURE:
                emit.Line("if (%s->Parameters().Length() < (sizeof(%s) + sizeof(uint32_t) + 1)) { return (0x80000000 | Core::ERROR_INVALID_INPUT_LENGTH); }" % (vars["message"], INSTANCE_ID))
                emit.Line()
        ...
        def EmitProxyMethodImplementation(index, method, interface_name, interface, retval, params,
            ...
            if proxy_params:
                if ENABLE_SECURE:
                    ...
                    emit.Line("if (completeResult != Core::ERROR_NONE) { return (0x80000000 | completeResult); }")
                else:

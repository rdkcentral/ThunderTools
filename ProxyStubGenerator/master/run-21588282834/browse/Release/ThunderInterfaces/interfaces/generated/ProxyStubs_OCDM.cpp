...
namespace ProxyStubs {
    ...
    static ProxyStub::MethodHandler ExchangeSessionStubMethods[] = {
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            hresult = [&]() -> Core::hresult {
                ...
                RPC::Data::Frame::Reader reader(message->Parameters().Reader());
                if (reader.Length() < (Core::RealSize<uint16_t>())) { return (COM_ERROR | Core::ERROR_READ_ERROR); }
                const uint16_t _parameter_de04e11aPeekedLen__ = reader.PeekNumber<uint16_t>();
                if (reader.Length() < (static_cast<uint32_t>(Core::RealSize<uint16_t>()) + _parameter_de04e11aPeekedLen__)) { return (COM_ERROR | Core::ERROR_READ_ERROR); }
                const string _parameter_de04e11a = reader.Text();
                if (reader.Length() < (Core::RealSize<uint16_t>())) { return (COM_ERROR | Core::ERROR_READ_ERROR); }
                const uint16_t _parameter_28e491efPeekedLen__ = reader.PeekNumber<uint16_t>();
                if (reader.Length() < (static_cast<uint32_t>(Core::RealSize<uint16_t>()) + _parameter_28e491efPeekedLen__)) { return (COM_ERROR | Core::ERROR_READ_ERROR); }
                const string _parameter_28e491ef = reader.Text();

                Exchange::OCDM_RESULT result = implementation->SetParameter(static_cast<const string&>(_parameter_de04e11a), static_cast<const string&>(_parameter_28e491ef));
    ...
    public:
        ...
        Exchange::OCDM_RESULT SetParameter(const string& _parameter_de04e11a, const string& _parameter_28e491ef) override
        {
            IPCMessage message(static_cast<const ProxyStub::UnknownProxy&>(*this).Message(13));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Text(_parameter_de04e11a);
            writer.Text(_parameter_28e491ef);

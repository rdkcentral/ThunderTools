 ...
 *
 * LocalNotification captures all delivered events into member variables.
 * Since the server dispatches notifications synchronously inside TriggerXxx,
 * all data is available immediately when TriggerXxx returns to the test.
 */
...
TEST_F(TestEvents, TriggerCompleted)
{
    ...
    EXPECT_EQ(_notif->completedCount, 1u);
    ASSERT_EQ(_proxy->TriggerCompleted(), Core::ERROR_NONE);
...
TEST_F(TestEvents, TriggerDisplayModesAvailable_WithModes)
{
